...
The packing agent hangs names on the claim (`--mention Name:kind` or a `TITLE`/`CLAIM`/`MENTION`/`AT` draft: `coherence pack --draft pack.txt`). After pack, loop: **observe** (`coherence check`), **reason**, **experiment** (`reject` + one replacement) until every atom PASSes and still looks true. The same loop runs on overlap (`coherence intersect` / `union`, including a topic with itself): check every challenge, reconstruct, compare with `--against` the previous packet. `TENSION` is a check FAIL (incompatible claims are not done). A mention is **garbage** when `grounding < 0.5`. Attestation is compact substring, name tokens, an `ALIAS` in the sentence, or initials of a title-case phrase. Check tells you the experiment: `anaphor:` → rewrite the claim with the name; `not attested` → put the name/`ALIAS` in the sentence or drop the join. Atom JSON shape is in [SPEC.md](../SPEC.md).
